...
def register_cli(subparser) -> None:
    ...
    export_backup = portability_subs.add_parser("export-backup", help="Create a portable bundle: JSONL export, SQLite backup, manifest, and checksums")
    export_backup.set_defaults(portability_action="export_backup", func=mann_memory_command)
    export_backup.add_argument("path", type=Path, help="Output directory or bundle path")
    export_backup.add_argument("--namespace", default="", help="Optional namespace filter")
    export_backup.add_argument("--include-archived", action="store_true", default=True, help="Include archived/deleted rows (default)")
    export_backup.add_argument("--active-only", dest="include_archived", action="store_false", help="Export only active status rows where supported")
