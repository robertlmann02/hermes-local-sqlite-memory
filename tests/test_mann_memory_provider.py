...
def test_store_and_search_memory(tmp_path):
    ...
    stored = json.loads(p.handle_tool_call("mann_memory_store", {
        "content": "The user prefers private Mann_Memory over cloud memory.",
...
def test_namespace_isolation(tmp_path):
    ...
    default = json.loads(p.handle_tool_call("mann_memory_search", {"query": "support", "namespace": "default"}))
    workspace_beta = json.loads(p.handle_tool_call("mann_memory_search", {"query": "support", "namespace": "workspace_beta"}))
...
def test_sync_turn_and_include_turns(tmp_path):
    ...
    result = json.loads(p.handle_tool_call("mann_memory_search", {
        "query": "local-host",
        "include_turns": True,
    }))
    assert result["turns"]
...
def test_review_proposal_approve_creates_memory(tmp_path):
    p = _provider(tmp_path)
    proposed = json.loads(p.handle_tool_call("mann_memory_review", {
        "action": "propose",
        "content": "The user prefers concise technical status updates.",
        "memory_type": "preference",
    }))
    pid = proposed["proposal"]["id"]
    approved = json.loads(p.handle_tool_call("mann_memory_review", {
        "action": "approve",
        "proposal_id": pid,
    }))
    assert approved["success"] is True
    assert approved["proposal"]["memory"]["content"].startswith("The user prefers")
    search = json.loads(p.handle_tool_call("mann_memory_search", {"query": "concise status"}))
    assert search["memories"]
...
def test_session_end_creates_review_proposals(tmp_path):
    p = _provider(tmp_path)
    p.on_session_end([
        {"role": "user", "content": "I prefer private Mann_Memory for Local."},
        {"role": "assistant", "content": "we decided to use SQLite FTS5 first."},
    ])
    result = json.loads(p.handle_tool_call("mann_memory_review", {"action": "list"}))
    contents = [x["content"] for x in result["proposals"]]
    assert any("private Mann_Memory" in c for c in contents)
...
def test_prefetch_returns_context(tmp_path):
    ...
    assert "Relevant Mann_Memory memories" in block
    assert "Assistant" in block


def test_graph_primitives_store_context_and_representation(tmp_path):
    p = _provider(tmp_path)
    peer = json.loads(p.handle_tool_call("mann_memory_graph", {
        "action": "upsert_peer",
        "peer_handle": "user",
        "role": "user",
    }))["peer"]
    session = json.loads(p.handle_tool_call("mann_memory_graph", {
        "action": "upsert_session",
        "session_id": "s-graph",
        "title": "Graph Mann_Memory design",
    }))["session"]
    msg = json.loads(p.handle_tool_call("mann_memory_graph", {
        "action": "add_message",
    ...
    con = json.loads(p.handle_tool_call("mann_memory_graph", {
    ...
    ctx = json.loads(p.handle_tool_call("mann_memory_graph", {
        "action": "context",
    ...
    assert ctx["messages"]
    representation = json.loads(p.handle_tool_call("mann_memory_graph", {
        "action": "build_representation",
        "peer_id": peer["id"],
    }))["representation"]
    assert "graph-style context" in representation["content"]
    status = json.loads(p.handle_tool_call("mann_memory_status", {}))["status"]
...
def test_dream_cycle_consolidates_messages_into_conclusions_and_representation(tmp_path):
    ...
    result = json.loads(p.handle_tool_call("mann_memory_graph", {
        "action": "dream",
        "namespace": "default",
        "peer_id": peer["id"],
        "limit": 10,
...
def test_dream_cycle_is_namespace_isolated(tmp_path):
    p = _provider(tmp_path)
    workspace_peer = json.loads(p.handle_tool_call("mann_memory_graph", {
        "action": "upsert_peer",
        "namespace": "workspace_beta",
        "peer_handle": "user",
        "role": "user",
    ...
    json.loads(p.handle_tool_call("mann_memory_graph", {
        "action": "dream",
        "namespace": "workspace_beta",
        "peer_id": workspace_peer["id"],
    }))
    default_ctx = json.loads(p.handle_tool_call("mann_memory_graph", {
        "action": "context",
        "namespace": "default",
        "query": "support ticket dream",
    }))["context"]
    workspace_ctx = json.loads(p.handle_tool_call("mann_memory_graph", {
        "action": "context",
        "namespace": "workspace_beta",
        "query": "support ticket dream",
    }))["context"]
...
def test_dream_cycle_does_not_recreate_archived_duplicate(tmp_path):
    ...
    for i in range(2):
        p.handle_tool_call("mann_memory_graph", {
            ...
            "peer_id": peer["id"],
            "role": "user",
            "content": f"The user wants recurring Mann_Memory dreaming evidence {i}.",
        })
    first = json.loads(p.handle_tool_call("mann_memory_graph", {
        "action": "dream",
        "namespace": "default",
        "peer_id": peer["id"],
    }))["dream"]
    assert first["created_conclusions"] == 1
    p.handle_tool_call("mann_memory_forget", {
        "memory_id": first["conclusions"][0]["id"],
        "mode": "archive",
    })
    # Archive the conclusion directly because mann_memory_forget is for durable memories.
    p._store.update_conclusion_status(first["conclusions"][0]["id"], "archived")
    second = json.loads(p.handle_tool_call("mann_memory_graph", {
        "action": "dream",
        "namespace": "default",
        "peer_id": peer["id"],
    }))["dream"]
...
def test_memory_guard_quarantines_prompt_injection_proposal(tmp_path):
    ...
    pending = json.loads(p.handle_tool_call("mann_memory_review", {"action": "list"}))
    assert pending["proposals"] == []
    quarantined = json.loads(p.handle_tool_call("mann_memory_review", {
        "action": "list",
        "status": "quarantined",
    }))
...
def test_memory_guard_quarantines_direct_store_and_avoids_active_search(tmp_path):
    ...
    search = json.loads(p.handle_tool_call("mann_memory_search", {"query": "sk-123456"}))
    assert search["memories"] == []
